...
//-----------------------------------------------------------------------------------------------------------
//RIPRISTINAVETZERO------------------------------------------------------------------------------------------
extern void ripristinaVetZero(long v[], int scarto,int grado);
/*void ripristinaVetZero(long v[], int scarto,int grado){
	int i;
  
    for(i = scarto;i< grado;i++){
        v[i] = 0;
    }
...
///----------------------------------------------------------------------------------------------------------
//TUTTIX-----------------------------------------------------------------------------------------------------
extern void tuttiX(long v[], long x, int j,int grado);
/*void tuttiX(long v[], long x, int j,int grado) {
	int i;
...
//-------------------------------------------------------------------------------------------------------------
//PRODUTTORIA--------------------------------------------------------------------------------------------------
extern void calcoloXastGradoMaggioreDueUnroll(type* x,int dim,int rig,long v[],int scarto,type* xast, int indiceXast,int grado,int t);
extern long calcoloXastGradoMaggioreDue(type* x,int dim,int rig,long v[],int scarto,type* xast, int indiceXast,int grado);
/*void calcoloXastGradoMaggioreDue(type* x,int dim,int rig,long v[],int scarto,type* xast, int indiceXast,int grado){
    int i;
    int offset;
    int indice;
...
void convert_data(params* input){
    ...
    input->xast=_mm_malloc((input->t*input->n)*sizeof(type),32);
    pxast=input->xast;
...
for(;i<input->n;i++){
          convert(i,pxast,px,grado,dim,input->degree,input->t);
}
...
void sgdNormal(params* input){
    ...
    input->theta=_mm_malloc(dimTheta*sizeof(type),32);
    pTheta=input->theta;
...
void sgdBatch(params* input){
    ...
    type* risultato=_mm_malloc(dimTheta*sizeof(type),32);
    input->theta=_mm_malloc(dimTheta*sizeof(type),32);
    pTheta=input->theta;
...
void sgdAdagrad(params* input){
    ...
    type* Gj=_mm_malloc((dimTheta*batch)*sizeof(type),32);
    type* risultato=_mm_malloc(dimTheta*sizeof(type),32);
